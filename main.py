...
Age = col1.slider('Enter Age',15,60) 
Gender = col2.radio("Select Gender",["Male", "Female", "Other"])
MaritalStatus = col3.selectbox("Choose which best describes your Marital Status",marital_list)
...
EmploymentStatus = col3.selectbox('Choose Which best describes your Employment Status', employmentStatus)
...
LoanDuration = col3.select_slider("How long do you intend to take to pay off your credit", loanDuration)
...
AppliedAmount = col3.number_input("The amount you wish to apply", 12345)
...
df_pred = pd.DataFrame([[Age, LoanDuration, NewCreditCustomer, VerificationType, Gender,
                          Interest, UseOfLoan, Amount, AppliedAmount,LanguageCode,Education,
...
if st.button('Click for Prediction'):
    if prediction[0] == 0:
        st.write('<p class="big-font">Your Credit Score is High.You are a Creditworthy Borrower.<span class="thumbs-up">&#128077;</span></p>',
                unsafe_allow_html=True)
    else:
        st.write('<p class="big-font">Your Credit Score is Low.You are not a Creditworthy Borrower.<span class="thumbs-down">&#128078;</span></p>',
                unsafe_allow_html=True)
